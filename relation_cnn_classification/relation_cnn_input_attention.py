...
import dependency_load_data_entity
import data_helpers
...
def train(argv=None):
    # load data
    print("Loading data ... ")
    x_train,y_train,entity_train= dependency_load_data_entity.load_train_data()
    x_test,y_test,entity_test = dependency_load_data_entity.load_test_data()

    # concatenate  and shuffle .
    x_sum = numpy.concatenate((x_train,x_test))
    y_sum = numpy.concatenate((y_train,y_test))
    entity_sum = numpy.concatenate((entity_train,entity_test))
    ...
    entity_shuffled = entity_sum[shuffle_indices]

    # split to train and test .
    x_train = x_shuffled[1000:]
    y_train = y_shuffled[1000:]
    entity_train = entity_shuffled[1000:]
    x_test=x_shuffled[:1000]
    y_test=y_shuffled[:1000]
    entity_test = entity_shuffled[:1000]

    print(x_train.shape)
    print(x_test.shape)
    print(entity_train.shape)
    # input is sentence
    # [n,embed]
    ...
    # [2] = [e1,e2]
    entity_node = tf.placeholder(tf.int32,shape=(2,))

    dropout_keep_prob = tf.placeholder(tf.float32,name="dropout_keep_prob")

    # input attention weight
    entity_w_weight = tf.Variable(
      tf.truncated_normal([max_document_length, EMBEDDING_SIZE],
                          stddev=0.1,
                          seed=SEED,
                          dtype=tf.float32))
    entity_u_weight = tf.Variable(
      tf.truncated_normal([max_document_length, EMBEDDING_SIZE],
                          stddev=0.1,
                          seed=SEED,
                          dtype=tf.float32))
    entity_v_weight = tf.Variable(
      tf.truncated_normal([max_document_length],
                          stddev=0.1,
                          seed=SEED,
                          dtype=tf.float32))
    ...
    # entitys
    def model(data,entitys):
        ids = [tf.squeeze(e) for e in tf.split(0,2,entitys)]
        # [e1,e2]
        entity_embeddings = [data[e] for e in ids]
        words = [tf.squeeze(word) for word in tf.split(0,max_document_length,data)]
        A_1 = []
        A_2 = []
        for word in words:
            # temp = [n,1]
            temp1 = tf.tanh(tf.matmul(entity_w_weight,tf.expand_dims(entity_embeddings[0],-1))+tf.matmul(entity_u_weight,tf.expand_dims(word,-1)))
            temp2 = tf.tanh(tf.matmul(entity_w_weight,tf.expand_dims(entity_embeddings[1],-1))+tf.matmul(entity_u_weight,tf.expand_dims(word,-1)))
            # dot product
            value1 = tf.reduce_sum(tf.mul(entity_v_weight,tf.squeeze(temp1)))
            value2 = tf.reduce_sum(tf.mul(entity_v_weight,tf.squeeze(temp2)))
            A_1.append(value1)
            A_2.append(value2)
        # softmax to matrix A_1 , A_2
        alpha_1 = tf.nn.softmax(tf.pack(A_1))
        alpha_2 = tf.nn.softmax(tf.pack(A_2))
        alpha = (alpha_1+alpha_2) / 2
        # apply alpha weight to words
        word_alpha = [tf.squeeze(score) for score in tf.split(0,max_document_length,alpha)]
        weighted_words = [tf.mul(word_alpha[i],words[i]) for i in range(max_document_length)]
        # data = [max_document_length,EMBEDDING_SIZE]
        data = tf.pack(weighted_words)
        # R = [d_c,n]
    ...
    w_o = model(train_data_node,entity_node)
    ...
    regularizers = tf.nn.l2_loss(wf_weights) + tf.nn.l2_loss(wf_biases) + tf.nn.l2_loss(u_weights) + tf.nn.l2_loss(classes_matrix) \
                   + tf.nn.l2_loss(entity_u_weight) + tf.nn.l2_loss(entity_v_weight) + tf.nn.l2_loss(entity_w_weight)
    ...
    def dev_step(x_batch,y_batch,entity_batch,best_test_loss,sess):
        ...
        for i in range(test_size):
            one_feed_dict = {train_data_node: x_batch[i],train_labels_node: y_batch[i],entity_node:entity_batch[i],dropout_keep_prob:1.0}
    ...
    with tf.Session() as sess:
        ...
        print('Initialized!')
        # Generate batches
        batches = data_helpers.batch_iter(list(zip(x_train,y_train,entity_train)),BATCH_SIZE,NUM_EPOCHS)
        # batch count
        batch_count = 0
        best_test_loss = None
        ...
        for batch in batches:
            ...
            if batch_count % EVAL_FREQUENCY == 0:
                print("\nEvaluation:")
                best_test_loss=dev_step(x_test,y_test,entity_test,best_test_loss,sess)
                print("")
            else:
                if batch_count % META_FREQUENCY == 99:
                    x_batch, y_batch, entity_batch = zip(*batch)
                    ...
                    for i in range(train_size):
                        feed_dict = {train_data_node: x_batch[i],train_labels_node: y_batch[i],entity_node:entity_batch[i],dropout_keep_prob:0.5}
