...
def train(argv=None):
    ...
    indic_true = tf.placeholder(tf.int32,shape=(None))
    indic_incorrect = tf.placeholder(tf.int32,shape=(None))
    ...
    true_values = tf.gather(tf.reshape(logits,[-1]),indic_true)
    neg_values = tf.gather(tf.reshape(logits,[-1]),indic_incorrect)
    ...
    loss = tf.reduce_mean(1.0 - true_values + neg_values)
    ...
    def dev_step(x_batch,y_batch,best_test_loss,sess):
        ...
        indexs,top_values,top_indices,acc,y_label,y_predict = sess.run([true_index,top_2_values, top_2_indices,train_accuracy,train_label,train_predict],feed_dict=feed_dict)
        # parse indexs
        true_indices = []
        for idx, label in enumerate(indexs):
            index = idx * NUM_CLASSES + label
            true_indices.append(index)

        incorrect_indices = []
        for idx, row in enumerate(top_indices.tolist()):
            if indexs[idx] == row[0]:
                target = row[1]
            else:
                target = row[0]
            index = idx * NUM_CLASSES + target
            incorrect_indices.append(index)
        feed_dict = {indic_true: numpy.asarray(true_indices), indic_incorrect: numpy.asarray(incorrect_indices),train_data_node: x_batch,train_labels_node: y_batch,dropout_keep_prob:1.0}
        summary,step, losses, lr = sess.run([merged,global_step, loss,learning_rate], feed_dict=feed_dict)
        ...
        for batch in batches:
            ...
            if batch_count % EVAL_FREQUENCY == 0:
                ...
                if  batch_count % META_FREQUENCY == 99:
                    ...
                    indexs,top_values, top_indices, acc = sess.run([true_index,top_2_values, top_2_indices,train_accuracy],feed_dict=feed_dict)
                    # parse indexs
                    true_indices = []
                    for idx, label in enumerate(indexs):
                        index = idx * NUM_CLASSES + label
                    ...
                    for idx , row in enumerate(top_indices.tolist()):
                        ...
                        index = idx * NUM_CLASSES + target
                        incorrect_indices.append(index)
                    feed_dict = {indic_true: numpy.asarray(true_indices), indic_incorrect: numpy.asarray(incorrect_indices),train_data_node: x_batch,train_labels_node: y_batch,dropout_keep_prob:0.5}
                    _, summary, step, losses = sess.run([train_op,merged,global_step, loss],
                                                            feed_dict=feed_dict,
                                                            options=run_options,
                    ...
                    for idx , row in enumerate(top_indices.tolist()):
                        ...
                        incorrect_indices.append(index)
                    feed_dict = {indic_true: numpy.asarray(true_indices), indic_incorrect: numpy.asarray(incorrect_indices),train_data_node: x_batch,train_labels_node: y_batch,dropout_keep_prob:0.5}
                    _, summary, step, losses = sess.run([train_op,merged,global_step, loss],feed_dict=feed_dict)
                    train_writer.add_summary(summary, step)
                    time_str = datetime.datetime.now().isoformat()
                    print("{}: step {}, loss {:g}, acc {:g}".format(time_str, step, losses,acc))
